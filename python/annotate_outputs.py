...
if args.snp_lookup:
    ...
    # intersect lookup table with variant_ids (col 6 in permutation_results; col 3 in snp_lookup)
    cmd = "awk 'NR==FNR {v[$6]; next} $3 in v' <(zcat "+args.permutation_results+") <(zcat "+args.snp_lookup+") > lookup_subset.txt"
    subprocess.call(cmd, shell=True, executable='/bin/bash')
    snp_lookup_df = pd.read_csv('lookup_subset.txt', index_col=2, sep='\t',
